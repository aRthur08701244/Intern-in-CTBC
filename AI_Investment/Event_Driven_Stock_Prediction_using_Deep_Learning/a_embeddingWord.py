        ...
        if os.path.exists(cc_matrix):
            ...
            for sentence in sentences:
                ...
                for i in range(n):
                    ...
                    # print(i, wi, start, end, self.context_sz)
...
def get_reuters_data(n_vocab):
    ...
    ## load stockName_abbr.txt ------------------------------------------------------------------------
    with open('./input2/stockName_abbr.txt') as data_file:
        stockName = data_file.readlines()

    for i in stockName:
        com = i.split(' ')[0]
        word2idx[com] = current_idx
        idx2word.append(com)
        word_idx_count[current_idx] = float('inf')
        current_idx += 1
    ## load stockName_abbr.txt ------------------------------------------------------------------------
    ...
    for i, j in itertools.product(range(2013, 2022), range(1, 13)):
        # news = pd.read_csv('./input/WB_SegAndRawNews/'+str(i)+str(j)+'_cnyesnews.csv')
        news = pd.read_csv('./input2/EB_SegAndRawNews/'+str(i)+str(j)+'_cnyesnews.csv') # 改成EB_SegAndRawNews，最新版本的斷詞
        for sentence in news['segmented']:
    ...
    # missing
    missing = list(range(n_vocab+1))
    for i in list(word2idx_small.values()):
        missing.remove(i)
    for i in missing:
        word2idx_small['缺失'+str(i)] = i

    return sentences_small, word2idx_small

def main(we_file, w2i_file, sen):
    # cc_matrix = "./input/cc_matrix.npy"
    cc_matrix = "./input2/cc_matrix.npy"
    if not os.path.isfile(w2i_file):
        sentences, word2idx = get_reuters_data(n_vocab=5000)
...
if __name__ == '__main__':
    ...
    we = './input2/glove_model_50.npz'
    w2i = './input2/word2idx.json'
    sen = './input2/sentences.json'
    main(we, w2i, sen)
